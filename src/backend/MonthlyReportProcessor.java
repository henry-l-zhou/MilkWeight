  ...
  private void stats() {
    ...
    for (MilkWeight mw : monthData) {
      if (weights.containsKey(mw.getFarmId())) { // if the farmId has already been seen
        ...
        if (mw.getWeight() > max.get(mw.getFarmId())) max.put(mw.getFarmId(), mw.getWeight()); // update max
        if (mw.getWeight() < min.get(mw.getFarmId())) min.put(mw.getFarmId(), mw.getWeight()); // update min
